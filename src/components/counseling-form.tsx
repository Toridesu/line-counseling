...
const CounselingForm = () => {
  ...
  return (
    <div className='bg-white text-gray-800 rounded-2xl shadow-2xl overflow-hidden'>
      <div className='p-8'>
        {/* エラーメッセージがある場合に表示 */}
        {state.message && <div className={`mb-4 p-3 rounded ${state.success ? 'bg-green-100 text-green-700' : 'bg-red-100 text-red-700'}`}>{state.message}</div>}
        <form action={formAction} className='space-y-6'>
          ...
          <div className='flex space-x-4'>
            ...
            <div className='w-1/2 space-y-2'>
              ...
              <select id='time' name='time' defaultValue='10:00' required className='flex h-10 w-full rounded-md border border-input bg-background px-3 py-2 text-sm ring-offset-background file:border-0 file:bg-transparent file:text-sm file:font-medium placeholder:text-muted-foreground focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring focus-visible:ring-offset-2 disabled:cursor-not-allowed disabled:opacity-50'>
          ...
          <div className='space-y-2'>
            ...
            <Textarea id='goals' name='goals' placeholder='プログラミングを学ぶ目的や、ご質問などをお聞かせください' className='h-28' />
          </div>

          <Button type='submit' className='w-full bg-blue-800 hover:bg-blue-900 text-white !py-8 rounded-lg font-bold text-lg transition-colors shadow-lg'>
            カウンセリングを予約する
          </Button>
        </form>
